...
app.get('/api/search', (req, res) => {
    try {
        ...
        const sortOrd = typeof sort_order === 'string' &&
...
app.get('/api/meta', (_req, res) => {
    try {
        if (!metaCache) {
            ...
            const unique = (values) => [...new Set(values.map((v) => v.trim()).filter(Boolean))].sort((a, b) => a.localeCompare(b, undefined, { sensitivity: 'base' }));
            metaCache = {
...
const tools = [
    {
        name: 'search_wines',
        description: 'Search wines by full-text query across wine names, brands, reviews, regions, AVAs, and varietals',
        input_schema: {
            type: 'object',
            properties: {
                query: {
    ...
    {
        name: 'filter_wines',
        description: 'Filter wines by specific criteria with comparison operators. Use for queries with specific requirements (price, rating, region, varietal, etc.)',
        input_schema: {
            type: 'object',
            properties: {
                filters: {
                    type: 'object',
                    description: 'Key-value pairs for filtering. Use column names as keys (mainVarietal, type, region, ava, brandName, price, rating, vintage, publicationDate, tastingDate). For numeric/date columns, use operators like ">4", "<50", ">=2012"',
                },
                limit: {
                    type: 'number',
    ...
    },
    {
        ...
        description: 'Get detailed information about a specific wine by name. Use when user asks for a specific wine.',
        input_schema: {
            type: 'object',
            properties: {
                wine_name: {
                ...
                exact_match: {
                    type: 'boolean',
                    description: 'If true, requires exact match. If false (default), allows partial matches',
                },
            },
            required: ['wine_name'],
        },
...
AVA, main varietal, type (Red/White/Rosé/etc.), tasting notes, tasting/publication dates.`;
        ...
        while (response.stop_reason === 'tool_use') {
            ...
            try {
                switch (toolUse.name) {
                    case 'search_wines':
                        toolResult = searchWines(wines, toolUse.input);
                        break;
                    case 'filter_wines':
                        toolResult = filterWines(wines, toolUse.input);
                        break;
                    case 'get_wine_details':
                        toolResult = getWineDetails(wines, toolUse.input);
                        break;
                    default:
        ...
        const finalMessage = textContent && textContent.type === 'text' ? textContent.text : '';
